...
float AudioSDL::get_volume() {
  return volume;
}

void AudioSDL::set_volume(float _volume) {
...
void AudioSDL::volume_up() {
  float vol = get_volume();
  set_volume(vol + 0.1f);
}

void AudioSDL::volume_down() {
  float vol = get_volume();
  set_volume(vol - 0.1f);
}

Audio::PTrack AudioSDL::PlayerSFX::create_track(int track_id) {
  PBuffer wav;
...
void AudioSDL::PlayerSFX::enable(bool enable) {
  enabled = enable;
  if (!enabled) {
    stop();
  }
...
void AudioSDL::PlayerSFX::stop() {
  Mix_HaltChannel(-1);
}

float AudioSDL::PlayerSFX::get_volume() {
  return static_cast<float>(Mix_Volume(-1, -1)) /
         static_cast<float>(MIX_MAX_VOLUME);
}

void AudioSDL::PlayerSFX::set_volume(float volume) {
...
void AudioSDL::PlayerSFX::volume_up() {
  set_volume(get_volume() + 0.1f);
}

void AudioSDL::PlayerSFX::volume_down() {
  set_volume(get_volume() - 0.1f);
}
...
void AudioSDL::TrackSFX::play() {
  int r = Mix_PlayChannel(-1, chunk, 0);
  if (r < 0) {
    std::cout << "Could not play SFX clip: "
                                  << Mix_GetError();
  }
}
...
AudioSDL::PlayerMIDI::PlayerMIDI() {
  if (current_midi_player != nullptr) {
    throwl("Only one midi player is allowed");
  }
...
Audio::PTrack AudioSDL::PlayerMIDI::create_track(int track_id) {
  ...
  //    std::cout << "Failed to initialize MOD music decoder";
  //    return nullptr;
  //  }
  //}
...
Audio::PTrack AudioSDL::PlayerMIDI::play_track(int track_id) {
  Audio::PTrack track;
  ...
  while (!track) {
    ...
    std::cout << "Playing MIDI track: " << current_track;
...
void AudioSDL::PlayerMIDI::enable(bool enable) {
  enabled = enable;
  if (!enabled) {
    stop();
  }
...
void AudioSDL::PlayerMIDI::stop() {
  Mix_HaltMusic();
}

float AudioSDL::PlayerMIDI::get_volume() {
  return static_cast<float>(Mix_VolumeMusic(-1)) /
         static_cast<float>(MIX_MAX_VOLUME);
}

void AudioSDL::PlayerMIDI::set_volume(float volume) {
  volume = std::max(0.f, std::min(volume, 1.f));
  float mix_volume = static_cast<float>(MIX_MAX_VOLUME) * volume;
  Mix_VolumeMusic(static_cast<int>(mix_volume));
}

void AudioSDL::PlayerMIDI::volume_up() {
  set_volume(get_volume() + 0.1f);
}

void AudioSDL::PlayerMIDI::volume_down() {
  set_volume(get_volume() - 0.1f);
}

AudioSDL::PlayerMIDI * AudioSDL::PlayerMIDI::current_midi_player = nullptr;

void AudioSDL::PlayerMIDI::music_finished_hook() {
  if (current_midi_player != nullptr) {
...
void AudioSDL::PlayerMIDI::music_finished() {
  if (is_enabled()) {
...
void AudioSDL::TrackMIDI::play() {
  int r = Mix_PlayMusic(chunk, 0);
  if (r < 0) {
    std::cerr << "Could not play MIDI track: "
                                 << Mix_GetError();
