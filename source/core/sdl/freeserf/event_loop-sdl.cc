...
EventLoopSDL::EventLoopSDL()
  ...
  if (eventUserTypeStep == (Uint32)-1) {
    throwl("Failed to register SDL event");
  }
  eventUserTypeStep++;
}

Uint32 EventLoopSDL::timer_callback(Uint32 interval, void *param) {
...
void EventLoopSDL::deferred_call(DeferredCall call, void *data) {
...
void EventLoopSDL::run() {
  SDL_TimerID timer_id = SDL_AddTimer(1, timer_callback, this);
...
void EventLoopSDL::zoom(float delta) {
  Graphics &gfx = Graphics::get_instance();
...
Timer * Timer::create(unsigned int _id, unsigned int _interval,
