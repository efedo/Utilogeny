...
#include "Utilogeny/source/core/sdl/freeserf/video-sdl.h"
...
VideoSDL::VideoSDL() {
  ...
  if (window == NULL) {
    throwl("Unable to create SDL window");
  }
  ...
  if (renderer == NULL) {
    throwl("Unable to create SDL renderer");
  }
...
SDL_Surface * VideoSDL::create_surface(int width, int height) {
  SDL_Surface *surf = SDL_CreateRGBSurface(0, width, height, bpp,
                                           Rmask, Gmask, Bmask, Amask);
  if (surf == nullptr) {
    throwl("Unable to create SDL surface");
  }

  return surf;
}

void VideoSDL::set_resolution(unsigned int width, unsigned int height, bool fs) {
  /* Set fullscreen mode */
  int r = SDL_SetWindowFullscreen(window,
                                  fs ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
  if (r < 0) {
    throwl("Unable to set window fullscreen");
  }
  ...
  if (r < 0) {
    throwl("Unable to set logical size");
  }

  fullscreen = fs;
}

void VideoSDL::get_resolution(unsigned int *width, unsigned int *height) {
  int w = 0;
  int h = 0;
...
Video::Frame * VideoSDL::get_screen_frame() {
  return screen;
}

Video::Frame * VideoSDL::create_frame(unsigned int width, unsigned int height) {
  Video::Frame *frame = new Video::Frame;
  frame->texture = create_texture(width, height);
  return frame;
}

void VideoSDL::destroy_frame(Video::Frame *frame) {
  SDL_DestroyTexture(frame->texture);
  delete frame;
}

Video::Image * VideoSDL::create_image(void *data, unsigned int width, unsigned int height) {
...
void VideoSDL::destroy_image(Video::Image *image) 
{
  SDL_DestroyTexture(image->texture);
  delete image;
}

void VideoSDL::warp_mouse(int x, int y) 
{
  SDL_WarpMouseInWindow(nullptr, x, y);
}

SDL_Surface * VideoSDL::create_surface_from_data(void *data, int width, int height)
{
...
SDL_Texture * VideoSDL::create_texture(int width, int height)
{
  ...
  if (texture == nullptr) {
    throwl("Unable to create SDL texture");
  }
...
VideoSDL::create_texture_from_data(void *data, int width, int height) {
  ...
  if (texture == nullptr) {
    throwl("Unable to create SDL texture from data");
  }
...
void VideoSDL::draw_image(const Video::Image *image, int x, int y, int y_offset, Video::Frame *dest) 
{
  ...
  if (r < 0) {
    throwl("RenderCopy error");
  }
}

void VideoSDL::draw_frame(int dx, int dy, Video::Frame *dest, int sx, int sy, Video::Frame *src, int w, int h) 
{
...
void VideoSDL::draw_rect(int x, int y, unsigned int width, unsigned int height, const Video::Color color, Video::Frame *dest)
{
  /* Draw rectangle. */
...
void VideoSDL::fill_rect(int x, int y, unsigned int width, unsigned int height, const Video::Color color, Video::Frame *dest) 
{
  ...
  if (r < 0) {
    throwl("RenderFillRect error");
  }
}

void VideoSDL::draw_line(int x, int y, int x1, int y1, const Video::Color color, Video::Frame *dest) 
{
  SDL_SetRenderTarget(renderer, dest->texture);
  SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, 0xff);
...
void VideoSDL::set_cursor(void *data, unsigned int width, unsigned int height) 
{
  if (cursor != nullptr) {
...
bool VideoSDL::set_zoom_factor(float factor) 
{
...
void VideoSDL::get_screen_factor(float *fx, float *fy)
{
  int w = 0;
  int h = 0;
